...
class TokenController {
  //definindo a classe TokenController
  async store(req, res) {
    ...
    const user = await User.findOne({ where: { email } }); //procurando o usuario pelo email - findoneone retorna o primeiro que encontrar
    ...
    if (!(await user.passwordisValid(password))) {
      //verificando se a senha é válida q setamos no user.js
